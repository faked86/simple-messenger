...
from server.oauth2.core import create_access_token
from server.api.api_models import UserIn
from server.api.auth.utils import hash_password, verify
...
async def delete_user(user: User, db: AsyncSession) -> None:
    ...
    await db.commit()
    logger.info(f"Deleted user with id: {user.id} from DB.")


async def get_token(user_credentials: OAuth2PasswordRequestForm,
    ...
    logger.info("Generated access_token")
